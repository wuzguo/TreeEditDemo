...
type
  ...
  TAESExpandedKey = array[0..43] of longword;
  ...
  public
    ...
    function StringEncrypt(value: string): string;
    function StringDecrypt(value: string): string;
  end;

// Key expansion routines for encryption

procedure ExpandAESKeyForEncryption(const Key: TAESKey;
  var ExpandedKey: TAESExpandedKey); overload;
// Block encryption routines

procedure EncryptAES(const InBuf: TAESBuffer; const Key: TAESExpandedKey;
  var OutBuf: TAESBuffer); overload;

// Stream encryption routines (ECB mode)

function EncryptAESStreamECB(Source: TStream; Count: cardinal;
  const Key: TAESKey; Dest: TStream): Boolean; overload;
function EncryptAESStreamECB(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey; Dest: TStream): Boolean; overload;

// Key transformation routines for decryption

procedure ExpandAESKeyForDecryption(var ExpandedKey: TAESExpandedKey); overload;
procedure ExpandAESKeyForDecryption(const Key: TAESKey;
  var ExpandedKey: TAESExpandedKey); overload;
// Block decryption routines

procedure DecryptAES(const InBuf: TAESBuffer; const Key: TAESExpandedKey;
  var OutBuf: TAESBuffer); overload;

// Stream decryption routines (ECB mode)

function DecryptAESStreamECB(Source: TStream; Count: cardinal;
  const Key: TAESKey; Dest: TStream): Boolean; overload;
function DecryptAESStreamECB(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey; Dest: TStream): Boolean; overload;
...
const
  gc_str_table =
    '$eZba1sWrtq5yoK%LhjvE,8klQw#m:<B2.!pNn>6x-z4AH3JM~_Rui)Pg0c]F;97/OTI^(+DdfSU@G&*{YCV?|`[}X';
...
var
  s_TableSize: integer;
  s_Table: array[0..255] of longword;
...
procedure ExpandAESKeyForEncryption(const Key: TAESKey; var ExpandedKey: TAESExpandedKey);
var
  I, J: integer;
  T: longword;
...
procedure ExpandAESKeyForDecryption(var ExpandedKey: TAESExpandedKey);
var
  I: integer;
...
procedure ExpandAESKeyForDecryption(const Key: TAESKey; var ExpandedKey: TAESExpandedKey);
begin
...
function EncryptAESStreamECB(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey; Dest: TStream): boolean;
var
...
function EncryptAESStreamECB(Source: TStream; Count: cardinal;
  const Key: TAESKey; Dest: TStream): Boolean;
var
  ExpandedKey: TAESExpandedKey;
begin
  ExpandAESKeyForEncryption(Key, ExpandedKey);
  result := EncryptAESStreamECB(Source, Count, ExpandedKey, Dest);
end;
...
begin
  ...
  result := DecryptAESStreamECB(Source, Count, ExpandedKey, Dest);
end;
...
constructor LAES.Create(key: string);
var
  i, x: integer;
  p: PChar;
begin
  inherited Create();
  FKey := key;
  if (s_TableSize = 0) then
  begin
    s_TableSize := Length(gc_str_table);
    p := PChar(gc_str_table);
    for i := 0 to s_TableSize - 1 do
    begin
      x := Integer(p[i]);
      s_Table[x] := i;
    end;
...
function LAES.StringEncrypt(value: string): string;
var
  stream: TMemoryStream;
  encrypt: TMemoryStream;
  dest: TMemoryStream;
  slen, elen, dlen, x, i, j, k, size: LongWord;
  AESKey: TAESKey;
  p: PLongWord;
  pDest: PChar;
  pTable: PChar;
begin
  ...
  stream.Size := 4 + Length(value);
  stream.Position := 4;
  stream.Write(value[1], Length(value));
  slen := stream.Size;
  elen := (slen + 15) div 16 * 16;
  stream.Position := 0;
  x := slen - 4;
  stream.WriteBuffer(x, SizeOf(x));
  encrypt := TMemoryStream.Create;
  encrypt.Size := elen;
  stream.Position := 0;
  encrypt.Position := 0;
  FillChar(AESKey, SizeOf(AESKey), 0);
  Move(PChar(FKey)^, AESKey, Min(SizeOf(AESKey), Length(FKey)));
  if (EncryptAESStreamECB(stream, 0, AESKey, encrypt) = False) then
  begin
    Result := '';
    exit;
  end;
  ...
  size := elen div 4;
  pDest := dest.Memory;
  FillChar(pDest^, dest.Size, 0);
  k := 0;
  pTable := PChar(gc_str_table);
  p := PLongWord(encrypt.Memory);
  for i := 0 to size - 1 do
  begin
    x := p^;
    for j := 0 to 4 do
    begin
      pDest[k] := pTable[x mod s_TableSize];
      x := x div s_TableSize;
      Inc(k);
    end;
    Inc(p);
...
function LAES.StringDecrypt(value: string): string;
var
  pSrc, p: PChar;
  encrypt: TMemoryStream;
  decrypt: TMemoryStream;
  size, i, j, x, y, z: LongWord;
  len: LongWord;
  pDec: PLongWord;
  AESKey: TAESKey;
begin
  ...
  size := Length(value);
  encrypt := TMemoryStream.Create;
  encrypt.Size := size * 4 div 5;
  pDec := PLongWord(encrypt.Memory);
  i := 0;
  while i < size do
  begin
    p := pSrc + i;
  ...
  len := encrypt.Size;
  len := (len + 15) div 16 * 16;
  decrypt := TMemoryStream.Create;
  decrypt.Size := len + 1;
  p := decrypt.Memory;
  FillChar(p^, decrypt.Size, 0);
  FillChar(AESKey, SizeOf(AESKey), 0);
  Move(PChar(FKey)^, AESKey, Min(SizeOf(AESKey), Length(FKey)));
  if (DecryptAESStreamECB(encrypt, 0, AESKey, decrypt) = false) then
  begin
    result := '';
    exit;
  end;
